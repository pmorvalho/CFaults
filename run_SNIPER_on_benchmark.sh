#!/usr/bin/env bash
#Title			: run_SNIPER_on_benchmark.sh
#Usage			: bash run_SNIPER_on_benchmark.sh -h
#Author			: pmorvalho
#Date			: March 28, 2024
#Description		: Runs SNIPER on an entire benchmark of instances.
#Notes			: 
...
process_reference_implementations(){
	...
	do
	    ...
	    echo "SNIPER: Dealing with $lab/$ex"
	    d=$inst_progs/$lab/$ex
	    # cp prints.* $d/.
	    num_unroll=$(cat $initial_dir/$tests_dir/$lab/$ex/$ex.unwind)
	    total=$((total+1))
	    mkdir -p $d
	    runsolver/src/runsolver -o $d/run.o -w $d/watcher.w -v $d/var.v -W $TIMEOUT --vsize-limit $MEMOUT --rss-swap-limit $MEMOUT --timestamp ./SNIPER.sh -i $progs/$lab/$ex".c" -cp -e $lab/$ex -nu $num_unroll -o $d -to $TIMEOUT -v &
	done
	wait
    done
}
...
process_C_Pack_IPAs_submissions(){
	...
	do
	    ...
	    do
		ex=$(echo $ex_dir | rev | cut -d '/' -f 1 | rev)
		echo "SNIPER: Dealing with $year/$lab/$ex"
		...
		do
		    ...
		    if [[ $REPR_SUBSET == 0 ]];
		    then
			runsolver/src/runsolver -o $d/$p/run.o -w $d/$p/watcher.w -v $d/$p/var.v -W $TIMEOUT  --vsize-limit $MEMOUT --rss-swap-limit $MEMOUT --timestamp ./SNIPER.sh -i $s -e $lab/$ex -nu $num_unroll -o $d/$p -v $INST_FLAGS -to $TIMEOUT &
		    else
			echo "Program: "$s
			runsolver/src/runsolver -o $d/$p/run.o -w $d/$p/watcher.w -v $d/$p/var.v -W $TIMEOUT  --vsize-limit $MEMOUT --rss-swap-limit $MEMOUT --timestamp ./SNIPER.sh -i $s -e $lab/$ex -nu $num_unroll -o $d/$p -v $INST_FLAGS -to $TIMEOUT
			break
...
process_TCAS_dataset(){
	...
	runsolver/src/runsolver -o $d/run.o -w $d/watcher.w -v $d/var.v -W $TIMEOUT  --vsize-limit $MEMOUT --rss-swap-limit $MEMOUT --timestamp ./SNIPER.sh -i $v_dir/tcas.c -nu $num_unroll -t -o $d -to $TIMEOUT -v
	return
    fi
	...
	d=$inst_progs/$v
	mkdir -p $d
	echo "SNIPER: Dealing with version $v"
	runsolver/src/runsolver -o $d/run.o -w $d/watcher.w -v $d/var.v -W $TIMEOUT  --vsize-limit $MEMOUT --rss-swap-limit $MEMOUT --timestamp ./SNIPER.sh -i $v_dir/tcas.c -nu $num_unroll -t -o $d -to $TIMEOUT -v
	total=$((total+1))
    done	
}
...
case $key in
    ...
    ;;    
    -h|--help)
	echo "USAGE: $0 [-e|--enum_all] [-c|--cpack_ipas] [-t|--tcas] [-rs|--repr_subset] [-to|--timeout] [-h|--help]
...
if [[ $CORRECT_PROG == 1 ]];
then
    echo "SNIPER: Checking the reference implementations"
    progs=$dataset/"reference_implementations"
    inst_progs=$tmp_dir/"C-Pack-IPAs/reference_implementations/SNIPER"
    process_reference_implementations
    echo
    echo

    echo "SNIPER: Checking the correct programs"
    progs=$dataset/"correct_submissions"
    inst_progs=$tmp_dir/"C-Pack-IPAs/correct_submissions/SNIPER"
    INST_FLAGS="-cp"
    process_C_Pack_IPAs_submissions
    echo
    echo
    ...
    summary
    echo "SNIPER: Computations on the correct programs are over."
else
    if [[ $TCAS == 1 ]];
    then
	dataset_dir="tcas"
	initial_dir=$(pwd)
	echo "SNIPER: Checking the TCAS dataset"	
	progs=$dataset_dir/"versions"
	inst_progs=$tmp_dir/"tcas/SNIPER"
	process_TCAS_dataset
	summary 
	echo "SNIPER: Computations on the TCAS dataset are over."
    else
	echo "SNIPER: Checking all incorrect submissions"
	progs=$dataset/"semantically_incorrect_submissions"
	inst_progs=$tmp_dir/"C-Pack-IPAs/incorrect_submissions/SNIPER"
	INST_FLAGS=""
	process_C_Pack_IPAs_submissions
	summary 
	echo "SNIPER: All incorrect programs have been analyzed!"
    fi
fi
